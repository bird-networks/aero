	...
	return {
			...
			for (const target of targets) {
				if (target.src && target.serveAt) {
					if (fs.existsSync(target.src)) {
						server.middlewares.use(target.serveAt, sirv(target.src, {
							dev: true,
							dotfiles: false,
							single: false,
						}));
						console.log(`Custom copy (dev): Serving ${target.src} at ${target.serveAt}`);
					} else {
						console.warn(`Custom copy (dev): Source directory ${target.src} for ${target.serveAt} not found. Skipping dev server setup.`);
					}
				}
			}
		},
		async writeBundle() {
			...
			for (const target of targets) {
				...
				if (target.src && target.serveAt) {
					destSubDir = target.serveAt.startsWith("/") ? target.serveAt.substring(1) : target.serveAt;
			...
			for (const op of finalCopyOps) {
				try {
					...
					console.log(`Custom copy (build): Copied from ${op.src} to ${op.dest}`);
				} catch (err) {
					console.warn(`Custom copy (build): Failed to copy from ${op.src} to ${op.dest}:`, err);
				}
			}
		}
	};
