...
import { getCurrentWispUrl } from "./wispBackendStore.js";
...
async function initBareMux(): Promise<void> {
	...
	try {
		...
		showErrorModal({
			...
			message:
				"BareMux failed to initialize properly. Please refresh the page and try again, if this still doesn't work, please report this in the Discord server.",
		});
		return;
	}

	try {
		const wispUrl = getCurrentWispUrl();
		await bareMuxConn.setTransport("/epoxy/index.mjs", [
			{
				wisp: wispUrl,
			},
		]);
		console.debug("[BareMux] BareMux transport set to Epoxy (Wisp)", wispUrl);
	} catch (err) {
		console.error("[BareMux] Failed to set BareMux transport:", err);
		showErrorModal({
			title: "Failed to Initialize BareMux Transport",
			message:
				"The BareMux transport failed to initialize. Please refresh the page and try again, if this still doesn't work, please report this in the Discord server.",
		});
		throw err;
	}
}
	...
	if ("serviceWorker" in navigator) {
		try {
			...
			showErrorModal({
				...
				message:
					"aero's SW registration failed, please refresh the page and try again, if this still doesn't work, please report this in the Discord server.",
			});
		}
	} else {
