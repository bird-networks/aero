 ...
 */
export default async function handleSW(
	event: FetchEvent
	...
	if (troubleshootRes.isErr())
		...
		return nErrAsync(troubleshootRes.error);

	// Develop a context
	const req = event.request;
	const reqUrl = new URL(req.url);
	const reqParams = reqUrl.searchParams;
	const reqDest = req.destination;
	...
	const isNavigate =
		req.mode === "navigate" && ["document", "iframe"].includes(reqDest);
	...
	if (isScript) {
		...
		if (isModParamStatus.passthroughParamExists)
			isMod = isModParamStatus.param === "true";
	}
	...
	const catchAllClientsValid =
		REQ_INTERCEPTION_CATCH_ALL === "clients" && event.clientId !== "";
	...
	if (clientUrlRes.isErr())
		return fmtNeverthrowErr(
			`${troubleshootingStrs.userErrTag}Failed to get the client URL. You have probably made a typo.`,
			clientUrlRes.error
		) as ResultAsync<Response, Error>;
	...
	const rewrittenReqValsRes = await rewriteReq({
		...
		reqUrl,
		clientId: event.clientId,
		aeroPathFilter: (reqPath: string): boolean => reqPath.includes("aero"),
		bundlesPath: "/bundles",
	...
	if (rewrittenReqValsRes.isErr())
		return fmtNeverthrowErr(
			"Failed to rewrite the request",
			rewrittenReqValsRes.error
		) as ResultAsync<Response, Error>;
	...
	const successReqVals = rewrittenReqVals as {
		cacheMan: CacheManager | undefined;
		rewrittenReqOpts: RequestInit;
		proxyUrl: string;
	};
	const { rewrittenReqOpts, proxyUrl } = successReqVals;
	// TODO: Use `Nullable` from `option-t`
	const cacheMan = successReqVals.cacheMan ?? null;

	// @ts-ignore
	if (typeof electronWebViewControls !== "undefined") {
		if (
			"httpReferrer" in electronWebViewControls &&
			electronWebViewControls.httpReferrer
		)
			(rewrittenReqOpts.headers as Headers).set(
				"Referer",
				electronWebViewControls.httpReferrer as string
			);
		if (
			"useragent" in electronWebViewControls &&
			electronWebViewControls.useragent
		)
			(rewrittenReqOpts.headers as Headers).set(
				"User-Agent",
				electronWebViewControls.useragent as string
			);
	}
	...
	if (rewrittenRespRes.isErr())
		return fmtNeverthrowErr(
			...
			rewrittenRespRes.error
		) as ResultAsync<Response, Error>;
	const { rewrittenBody, rewrittenRespHeaders, rewrittenStatus } =
		rewrittenRespRes.value;

	// Perform encoded body emulation
	if (
		ENC_BODY_EMULATION &&
		(typeof rewrittenBody === "string" ||
			rewrittenBody instanceof ArrayBuffer)
	)
		// This will modify the resp headers
		await perfEncBodyEmu(rewrittenBody, rewrittenRespHeaders);

	/** The rewritten response */
	const rewrittenResp = new Response(
		rewrittenStatus === 204 ? null : rewrittenBody,
		{
	...
	if (cacheMan !== null) {
		...
		for (const client of await clients.matchAll()) {
			if (isNavigate) {
				...
				try {
					proxyClientOrigin = new URL(afterPrefix(client.url, aeroConfig.prefix, self.logger)).origin;
				} catch (err) {
		...
		*/
		...
		if (CACHES_EMULATION) {
			const perfCacheSettingRes = await perfCacheSetting({
				...
				rewrittenResp,
				// @ts-ignore: This is created under an if statement of `CACHES_EMULATION`, so we are fine
				cacheMan,
				clientId: event.clientId
			});
			if (perfCacheSettingRes.isErr())
				return fmtNeverthrowErr(
					"Failed to cache the response",
					perfCacheSettingRes.error
				) as ResultAsync<Response, Error>;
		}
	}
...
self.routeAero = (event: FetchEvent): Result<boolean, Error> => {
	...
	if (troubleshootJustConfigsRes.isErr())
		...
		// TODO: Use `fmtNeverthrowErr`
		return nErr(new Error("Troubleshooting configuration check failed"));

	return nOk(
		event.request.url.startsWith(location.origin + aeroConfig.prefix)
	);
};
