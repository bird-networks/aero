 ...
 */
export function createEscapePropGetHandler(): Result<
	(escapedProps: readonly string[]) => ProxyHandler<any>,
	Error
> {
	if (
		typeof AERO_CLASS_MEMBER_ESCAPE_KEYWORD === "undefined" ||
		AERO_CLASS_MEMBER_ESCAPE_KEYWORD === ""
	) {
		return nErr(
			new Error(
				"Global constant AERO_CLASS_MEMBER_ESCAPE_KEYWORD is not defined or is empty. Cannot create escape property handler."
			)
		);
	}
		...
		(escapedProps: readonly string[]): ProxyHandler<any> => ({
			get(target: any, prop: string | symbol, receiver: any): any {
				...
				if (escapedProps.includes(stringProp)) {
					const unescapeResult = unescape(stringProp, {
						keyword: classMemberEscapeKeyword,
					});
					if (unescapeResult.isErr()) {
						$aero.logger.warn(
							`Failed to unescape property "${stringProp}": ${unescapeResult.error.message}`
						);
			...
			set(target: any, prop: string | symbol, value: any, receiver: any): boolean {
				const stringProp = String(prop);
				if (escapedProps.includes(stringProp)) {
					const escapeResult = escapeGetTrap(target, stringProp, {
						keyword: classMemberEscapeKeyword,
					});
					if (escapeResult.isErr()) {
						$aero.logger.warn(
							`Failed to create escaped key for property "${stringProp}": ${escapeResult.error.message}`
						);
...
export function escapeGetTrap(
	...
	if (typeof ESCAPE_METHOD === "undefined" || typeof ESCAPE_KEYWORD === "undefined") {
		return nErr(
			new Error(
				"Missing global feature flags: ESCAPE_METHOD or ESCAPE_KEYWORD. Unable to determine how properties should be escaped."
			)
		);
	}
	...
	if (ESCAPE_METHOD === "RegExp") {
		return nErr(
			new Error(
				'The ESCAPE_METHOD "RegExp" is a STUB for escapeGetTrap. Please set it to "JS".'
			)
		);
	}
	...
	if (ESCAPE_KEYWORD === "JS") {
		...
		return nErr(
			new Error(`Could not find unique escaped key for "${prop}" after 100 attempts.`)
		);
	}
	return nErr(
		new Error(`Unknown ESCAPE_KEYWORD "${ESCAPE_KEYWORD}" or ESCAPE_METHOD "${ESCAPE_METHOD}".`)
	);
}
...
export function unescape(
	...
	if (typeof ESCAPE_METHOD === "undefined" || typeof ESCAPE_KEYWORD === "undefined") {
		return nErr(
			new Error(
				"Missing global feature flags: ESCAPE_METHOD or ESCAPE_KEYWORD. Unable to determine how properties should be unescaped."
			)
		);
	}
	...
	if (ESCAPE_METHOD === "RegExp") {
		return nErr(
			new Error('The ESCAPE_METHOD "Regexp" is a STUB, so please set it to JS instead')
		);
	}
	...
	if (ESCAPE_METHOD === "JS") {
		...
		if (unescaped === undefined) {
			...
			return nErr(
				new Error(
					`Failed to unescape string "${str}" with keyword "${keyword}": pop returned undefined.`
				)
			);
		}
