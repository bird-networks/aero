...
const validateBeforeInstallPromptFakeEventResponse =
	typia.createValidate<BeforeInstallPromptFakeEventResponse>();
...
export default [
	{
		init() {
			const fakeEventData = $aero.sandbox.extLib.syncify(
				new Promise(resolve => {
					...
					beforeInstallPromptBc.onmessage = event => {
						...
						if (
							...
							event.data.for === "fake-event-response"
						) {
	...
	},
	{
		proxyHandler: {
			apply() {
				appBadges = appBadges.filter(
					appBadge => appBadge.proxyOrigin !== proxyLocation().origin
				);
